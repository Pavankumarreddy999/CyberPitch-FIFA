...
function TicketingFraud({ data }: { data: ThreatRecord[] }) {
  ...
  const priceComparison=useMemo(()=>{
    ...
    tickets.forEach(t=>{
      ...
      buckets[key].scamAvg+=(t.scamPrice??0); buckets[key].n+=1;
    });
    return Object.values(buckets).map(b=>({tier:b.tier, Official:b.official, "Scam avg":Math.round(b.scamAvg/b.n)}));
  },[tickets]);
    ...
    <div>
      <div className="stat-grid" style={{ gridTemplateColumns:"repeat(3,1fr)" }}>
        {[
          ...
          {label:"Estimated financial loss", value:totals.loss, icon:AlertTriangle, color:"var(--gold)", fmt:(v:number)=>`$${v.toLocaleString()}`},
          {label:"Estimated affected users", value:totals.victims, icon:Users, color:"var(--green)", fmt:(v:number)=>v.toLocaleString()},
        ].map((s,i)=>{
          const Icon=s.icon;
          return (
            <div className="card stat-card" key={i}>
          ...
          <div style={{ height:200 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={priceComparison}>
                ...
                <YAxis tick={{ fill:"#8ea0bc", fontSize:11 }} axisLine={false} tickLine={false}/>
                <Tooltip contentStyle={{ background:"rgba(8,17,36,0.95)", border:"1.5px solid var(--blue-neon)", borderRadius:10, fontSize:12 }} cursor={{ fill:"rgba(255,215,0,0.06)" }}/>
                <Bar dataKey="Official" fill="var(--green)" radius={[4,4,0,0]} barSize={16}/>
                <Bar dataKey="Scam avg" fill="var(--red)"   radius={[4,4,0,0]} barSize={16}/>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div style={{ height:200 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={flagFreq} layout="vertical" margin={{ left:0, right:20 }}>
                ...
                <Tooltip contentStyle={{ background:"rgba(8,17,36,0.95)", border:"1.5px solid var(--blue-neon)", borderRadius:10, fontSize:12 }} cursor={{ fill:"rgba(255,215,0,0.06)" }}/>
                <Bar dataKey="count" fill="var(--gold)" radius={[0,4,4,0]} barSize={12}/>
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
